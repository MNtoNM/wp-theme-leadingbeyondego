<?php get_header(); ?>
<div class="u-margin-bottom-medium"></div>
<div class="container">
  <div id="section-1">
    <h3 class="section-1__pullquote">
      <div class="u-margin-bottom-medium"></div>


      The human ego is not equipped<br />
      for the challenges of our time.<br />
      Ego IS the challenge.
    </h3>

    <h2 class="center white">A 9-week Interactive Leadership Course</h2>
    <h5 class="center white">with Steve Motenko, Barbara J Hunt and Rodney Dueck</h5>
    <h6 class="center white u-margin-bottom-medium">Presented by Leading Beyond Ego LLC</h6>
    <div>
      <div class="u-margin-bottom-medium"></div>
      <img src="<?php echo $result['image']; ?>"/>

      <a href="#learn-more" class="button button-light" id="nav-enroll">Learn More</a>

    </div>
  </div>
  <div class="u-padding-top-big"></div>
  <div class="u-padding-top-big"></div>

  <div id="section-2">
    <div class="u-padding-top-big"></div>
    <div class="u-margin-bottom-big"></div>

    <!-- Does this sound familiar? -->
    <?php $result = getSalescopy(40); ?>

    <div class="copy-block__outer">
      <div class="copy-block__inner">
        <h2><?php echo $result['headline'];?></h2>
        <p><?php echo $result['content'] ;?></p>
        <div class="u-padding-top-big"></div>
      </div>
    </div>
  </div>
  <!-- parallax -->
  <div class="parallax-window-min-30" data-parallax="scroll" data-image-src="<?php echo wp_get_attachment_image_src('170', 'full')[0] ?>" data-speed="0.3"></div>

  <div id="section-3">
    <div class="u-padding-top-big"></div>

    <!-- Ego. -->
    <?php $result = getSalescopy(43); ?>

    <div class="copy-block__outer">
      <div class="copy-block__inner">
        <h2><?php echo $result['headline'];?></h2>
        <p><?php echo $result['content'] ;?></p>
      </div>
    </div>
    <div class="u-padding-top-big"></div>
  </div>

  <div id="section-4">
    <div class="u-padding-top-big"></div>

    <div class="row">
      <div class="col-1-of-2">
        <!-- What if you could...  -->
        <?php $result = getSalescopy(45); ?>
        <div class="copy-block__outer">
          <div class="copy-block__inner">
            <h2><?php echo $result['headline'];?></h2>
            <p><?php echo $result['content'] ;?></p>
          </div>
        </div>
      </div>

      <div class="col-1-of-2">
        <img class="office-folks" alt="Colleagues conferring" src="<?php echo wp_get_attachment_image_src('171', 'full')[0] ?>" />
        <div class="u-margin-bottom-big"></div>
      </div>
    </div>
  </div>

  <div id="section-5">
    <div class="u-padding-top-big"></div>
    <!-- "Beyond" Ego -->
    <?php $result = getSalescopy(47); ?>
    <div class="copy-block__outer">
      <div class="copy-block__inner">
        <h2><?php echo $result['headline'];?></h2>
        <p><?php echo $result['content'] ;?></p>
      </div>
    </div>
    <div class="u-padding-top-big"></div>
  </div>

  <!-- road parallax -->
  <div class="parallax-window-min-40" data-parallax="scroll" data-image-src="<?php echo wp_get_attachment_image_src('172', 'full')[0] ?>" data-speed="0.5"></div>

  <div id="section-6">
    <!-- <div class="u-padding-top-big"></div> -->

    <!-- Testimonials (Might be moved later) -->
    <div class="section-testimonials">
      <?php $result = getSalescopy(284); ?>
      <div class="copy-block__outer">
        <div class="copy-block__inner">
          <h2><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>
        </div>
      </div>
      <div class="u-padding-top-big"></div>
    </div><!-- .section-testimonials -->

    <!-- Who is this course for? -->
    <div class="u-margin-bottom-medium"></div>
    <?php $result = getSalescopy(49); ?>
    <div class="copy-block__outer">
      <div class="copy-block__inner">
        <h2><?php echo $result['headline'];?></h2>
        <p><?php echo $result['content'] ;?></p>
      </div>
    </div>
    <div class="u-padding-top-big"></div>
  </div>

<!-- Parallax: fistbump -->
<div class="parallax-window-min-40" data-parallax="scroll" data-image-src="<?php echo wp_get_attachment_image_src('173', 'full')[0] ?>" data-speed="0.5"></div>

  <div id="section-7">
    <div class="u-padding-top-big"></div>
    <!-- Next Generation Leadership -->
    <?php $result = getSalescopy(51); ?>
    <div class="copy-block__outer">
      <div class="copy-block__inner">
        <h2><?php echo $result['headline'];?></h2>
        <p><?php echo $result['content'] ;?></p>
      </div>
    </div>
    <div class="u-padding-top-big"></div>
  </div>

<!-- Parallax: Compass -->
  <div class="parallax-window-min-40" data-parallax="scroll" data-image-src="<?php echo wp_get_attachment_image_src('174', 'full')[0] ?>" data-speed="0.5"></div>

  <div id="section-8">
    <div class="u-padding-top-big"></div>
    <!-- Who is the beyond-ego leader? -->
    <?php $result = getSalescopy(53); ?>
      <div class="copy-block__outer">
        <div class="copy-block__inner">
          <h2><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>
        </div>
      </div>
    <div class="u-padding-top-big"></div>
  </div>

<div id="section-9">
  <div class="row">
    <div class="col-1-of-2">
      <img class="office-folks" alt="Woman in office attire" src="<?php echo wp_get_attachment_image_src('175', 'full')[0] ?>" />
    </div>

    <div class="col-1-of-2">
      <!-- You're leading beyond ego when... -->
      <?php $result = getSalescopy(55); ?>
      <div class="copy-block__outer">
        <div class="copy-block__inner">
          <h2><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>
        </div>
      </div>

    </div>
  </div>
  <div class="u-padding-top-big"></div>
</div>


<div id="section-10">

  <div class="rule-horizontal"></div>
  <div class="u-padding-top-big"></div>
  <!-- An Invitation -->
  <?php $result = getSalescopy(57); ?>
  <div class="copy-block__outer">
    <div class="copy-block__inner">
      <h1><?php echo $result['headline'];?></h1>
      <div class="u-margin-bottom-medium"></div>
      <p class="center"><?php echo $result['content'] ;?></p>
      <div class="u-margin-bottom-medium"></div>

      <h4 class="center">Registration is currently closed.</h4>
      <div class="u-margin-bottom-medium"></div>

      <a href="#learn-more" class="button button-dark">Learn More</a>

    </div>
  </div>



  <div class="u-padding-top-big"></div>


  <div class="u-padding-top-big"></div>
  <div class="rule-horizontal"></div>

</div>

<div id="section-11">
  <div class="u-padding-top-big"></div>

  <h1>Course Modules</h1>
  <div class="u-margin-bottom-small"></div>

<?php echo render_course_modules([59, 63, 71, 74, 77, 80, 83, 86, 89]); ?>


  <div class="u-padding-top-big"></div>
</div>

<!-- <div id="section-12">
  <h4 class="center">Have Questions?</h4>
</div> -->

<div class="rule-horizontal"></div>

<div id="section-13">
  <div class="u-padding-top-big"></div>
  <h1>What's Included</h1>
  <div class="u-padding-top-big"></div>

</div>

<div id="section-14">
  <!-- What's included:  Nine 2-Hour Weekly Interactive Workshops  -->
  <?php $result = getSalescopy(93); ?>

      <h2 class="center"><?php echo $result['headline'];?></h2>
      <div class="row">
        <div class="col-1-of-3">
          <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
        </div>
        <div class="col-2-of-3">
          <p><?php echo $result['time'] ;?></p>
          <p><?php echo $result['content'] ;?></p>
        </div>
      </div>

    <div class="u-margin-bottom-medium"></div>
    <div class="rule-angled"></div>
    <div class="u-margin-bottom-big"></div>

  <!-- What's included: What's Included - Eight 90-Minute Q&A sessions  -->
  <?php $result = getSalescopy(95); ?>
  <h2 class="center"><?php echo $result['headline'];?></h2>
  <div class="row">
    <div class="col-2-of-3">
      <p><?php echo $result['content'] ;?></p>
    </div>
    <div class="col-1-of-3">
      <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
    </div>
    </div>

    <div class="u-margin-bottom-medium"></div>
    <div class="rule-angled"></div>
    <div class="u-margin-bottom-big"></div>

  <!-- What's included: Nine Power Principles and Practices for Leading Beyond Ego  -->
  <?php $result = getSalescopy(160); ?>
  <h2 class="center"><?php echo $result['headline'];?></h2>
  <div class="row">
    <div class="col-1-of-3">
      <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
    </div>
    <div class="col-2-of-3">
      <p><?php echo $result['content'] ;?></p>
    </div>
  </div>

    <div class="u-margin-bottom-medium"></div>
    <div class="rule-angled"></div>
    <div class="u-margin-bottom-big"></div>

  <!-- What's included: What's Included - The Workplace Benefits of Meditation  -->
  <?php $result = getSalescopy(102); ?>
    <h2 class="center"><?php echo $result['headline'];?></h2>
    <div class="row">
      <div class="col-2-of-3">
        <p><?php echo $result['content'] ;?></p>
      </div>
      <div class="col-1-of-3">
        <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
      </div>
    </div>

  <div class="u-margin-bottom-medium"></div>
  <div class="rule-angled"></div>
  <div class="u-margin-bottom-big"></div>



  <!-- What's included: What's Included - Weekly Study / Practice Guides  -->
  <?php $result = getSalescopy(104); ?>
  <h2 class="center"><?php echo $result['headline'];?></h2>
    <div class="row">
      <div class="col-1-of-3">
        <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
      </div>
      <div class="col-2-of-3">
        <p><?php echo $result['content'] ;?></p>
      </div>
    </div>

    <div class="u-margin-bottom-medium"></div>
    <div class="rule-angled"></div>
    <div class="u-margin-bottom-big"></div>

  <!-- What's included: A Private Online Learning Portal  -->
  <?php $result = getSalescopy(106); ?>
      <h2 class="center"><?php echo $result['headline'];?></h2>
      <div class="row">
        <div class="col-2-of-3"><p><?php echo $result['content'] ;?></p>
      </div>
      <div class="col-1-of-3">
        <img class="whats-included-photo" src="<?php echo $result['image']; ?>"/>
      </div>
    </div>

  <div class="u-padding-top-big"></div>



</div><!-- Section 14 -->

<div id="section-15">
  <div class="u-padding-top-big"></div>
  <h1 class="center">Support</h1>
  <div class="u-margin-bottom-medium"></div>

  <div class="copy-block__outer">
    <div class="copy-block__inner">
      <p>We want you to have an excellent experience of this course. If you have questions about any aspect of it, please contact us at <a href="mailto:support@leadingbeyondego.com">support@leadingbeyondego.com</a> and one of our team will get back to you. We strive to respond to all inquiries within 24 hours, Monday through Friday.</p>
    </div>
  </div>

  <div class="u-padding-top-big"></div>

  </div>

<div id="section-16">
  <div class="u-padding-top-big"></div>

  <h1>Faculty</h1>
  <div class="u-margin-bottom-medium"></div>
  <!-- Faculty: General Info  -->
  <?php $result = getSalescopy(108); ?>
  <div class="copy-block__outer">
    <div class="copy-block__inner">
      <p><?php echo $result['content'] ;?></p>
    </div>
  </div>

  <div class="u-margin-bottom-medium"></div>
  <div class="row">
    <div class="col-1-of-3">
      <!-- Faculty: Members: Steve:  -->
      <?php $result = getSalescopy(110); ?>

          <img class="faculty-image" src="<?php echo $result['image']; ?>"/>
          <h2 class="center"><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>

    </div>

    <div class="col-1-of-3">
      <!-- Faculty: Members: Barbara:  -->
      <?php $result = getSalescopy(113); ?>

          <img class="faculty-image" src="<?php echo $result['image']; ?>"/>
          <h2 class="center"><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>

    </div>

    <div class="col-1-of-3">
      <!-- Faculty: Members: Rod:  -->
      <?php $result = getSalescopy(116); ?>

          <img class="faculty-image center" src="<?php echo $result['image']; ?>"/>
          <h2 class="center"><?php echo $result['headline'];?></h2>
          <p><?php echo $result['content'] ;?></p>
        </div>

  </div>
  <div class="u-padding-top-big"></div>
</div>

<div class="rule-horizontal"></div>

<div id="section-17">
  <div class="u-padding-top-big"></div>
  <h1>Frequently Asked Questions</h1>
  <div class="u-padding-top-big"></div>

  <!-- FAQ: Q & A Accordion Template -->
  <div class="faq-accordion" id="faq-accordion">
    <?php echo render_questions([119, 121, 124, 136, 129, 131, 138, 140]); ?>


  </div>

</div>
<div class="u-margin-bottom-medium"></div>

<a href="#learn-more" class="button button-dark" id="nav-enroll-foot">Learn More</a>
<div class="u-padding-top-big"></div>
<div class="u-margin-bottom-medium"></div>

<?php get_footer(); ?>
</div>
